...
if(await exists(assetsDir)){ const assets=await readdir(assetsDir); expect(assets.includes('routine-scroll-poster.jpg'),'Missing routine poster.'); expect(assets.includes('routine-forward.mp4'),'Missing forward routine video.'); expect(assets.includes('routine-reverse.mp4'),'Missing reverse routine video.'); expect(assets.includes('footer-waves.svg'),'Missing footer wave pattern.'); expect(assets.includes('closing-wave.svg'),'Missing Home closing wave.'); expect(assets.includes('avokind-wordmark-clean.png'),'Missing AvoKind wordmark.'); const stepDir=join(assetsDir,'routine-steps-webp'); expect(await exists(stepDir),'Missing segmented routine animations.'); if(await exists(stepDir)){ for(const name of ['scoop-forward.webp','scoop-reverse.webp','stir-forward.webp','stir-reverse.webp','enjoy-forward.webp','enjoy-reverse.webp']) expect(await exists(join(stepDir,name)),`Missing routine animation: ${name}`); } }
