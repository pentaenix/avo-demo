...
if(await exists(assetsDir)){ const assets=await readdir(assetsDir); expect(assets.includes('routine-scroll-poster.jpg'),'Missing routine sequence poster.'); expect(assets.includes('routine-frames'),'Missing scroll-controlled routine frame sequence.'); expect(assets.includes('footer-waves.svg'),'Missing footer wave pattern.'); const framesDir=join(assetsDir,'routine-frames'); if(await exists(framesDir)){ const frames=(await readdir(framesDir)).filter(name=>/^frame_\d{3}\.webp$/.test(name)); expect(frames.length===130,`Expected 130 routine frames; found ${frames.length}.`);} expect(assets.includes('avokind-wordmark-clean.png'),'Missing AvoKind wordmark.'); }
